...
>>> m
MsgTestCaseSkip(_api_version = 1.0.9, description = Skip testcase, node = someNode, testcase_id = some_testcase_id, )
>>> m.routing_key
...
>>> m
MsgTestCaseSkip(_api_version = 1.0.9, description = Skip testcase, node = someNode, testcase_id = TD_COAP_CORE_03, )
>>> m.testcase_id
'TD_COAP_CORE_03'

# and even export the message in json format (for example for sending the message though the amqp event bus)
>>> m.to_json()
'{"_api_version": "1.0.9", "description": "Skip testcase", "node": "someNode", "testcase_id": "TD_COAP_CORE_03"}'
...
'testsuite.start'
>>> m.to_json()
'{"_api_version": "1.0.9", "description": "Test suite START command"}'
>>> json_message = m.to_json()
>>> obj=Message.load(json_message,'testsuite.start', None )
>>> obj
MsgTestSuiteStart(_api_version = 1.0.9, description = Test suite START command, )
...
>>> err = MsgErrorReply(m)
>>> err
MsgErrorReply(_api_version = 1.0.9, error_code = None, error_message = None, ok = False, )
...
class Message(object):
    ...
    def load(cls, json_body, routing_key, properties=None):
        ...
        >>> m.to_json()
        '{"_api_version": "1.0.9", "capture_id": "TD_COAP_CORE_01"}'
        >>> json_message = m.to_json()
        >>> json_message
        '{"_api_version": "1.0.9", "capture_id": "TD_COAP_CORE_01"}'
        >>> obj=Message.load(json_message,'testsuite.start', None )
